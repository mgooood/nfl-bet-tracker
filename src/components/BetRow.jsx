...
export default function BetRow({ bet, index, onUpdate }) {
  ...
  const [editAmount, setEditAmount] = useState(String(bet.amount))
  ...
  return (
    <article className="bet" aria-label={`Bet vs ${bet.opponent} on ${bet.date}`}>
      <div className="bet-grid">
        ...
        <div className="bet-field">
          ...
          {editing ? (
            ...
            <span className={bet.amount >= 0 ? 'value positive' : 'value negative'}>{formatCurrency(bet.amount)}</span>
          )}
        </div>
      </div>
